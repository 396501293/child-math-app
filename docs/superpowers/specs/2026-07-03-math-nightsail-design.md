...
- **实现策略：枚举 + 两段式加权抽样**。混合档（`mix`）先按 `weight` 选中子池，再在子池内穷举合法组合（单个子池最大约 2500 个、全库合计数万级，性能无虞）并均匀抽取；过滤 `recentKeys`（模式滚动去重，最近 5 题）。单一子池即退化为均匀抽样。若直接对合并域均匀抽样，band 15 的 60/40 会因子池大小悬殊（145 vs 36）跑成 80/20——禁止这种实现。
...
## 增补 v1.1（2026-07-04）：第四章「银河」乘法与进阶

应用户孩子反馈「太简单」增补。规则权威来源仍为 `题库难度与模式设计.md`（其第四章小节）。技术增量：

- `Op` 联合类型增加 `'×'`；`QuestionKind` 增加 `mul`、`missing-mul-a`、`missing-mul-b`；两步题（乘加/乘减）复用 chain3 机制（ops 含 `×`，从左往右求值）。
- 总关数 45 → **60**；`chapterOf` 返回 1–4；解锁上限、地图章节切换、结算「进入第四章 ▶」按钮、第 60 关无下一关，随之调整。持久化 schema 不变（`stars`/`unlocked` 天然兼容）。
- 计数块新增 `array-grid`（a 行 × b 列网格，仅纯乘法题；缺数/两步不出教具）。
